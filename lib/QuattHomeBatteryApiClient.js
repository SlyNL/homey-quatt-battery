...
class QuattHomeBatteryApiClient {
  ...
  async _pairHomeBattery(accessKeyUuid, serialNumber, checkCode) {
    this._homey.log('Attempting to pair home battery with:', { accessKeyUuid, serialNumber, checkCode });

    let status, data;
    try {
      const response = await this._auth.request('POST', '/me/devices/homeBattery/pair', {
      ...
      this._homey.error('Pair request exception:', err.message, err.stack);
      throw err;
    }

    if (![200, 201].includes(status) || !data) {
      const errorMsg = data?.message || data?.error || 'Unknown error';
      this._homey.error('Home battery pair failed: status=', status, 'error=', errorMsg, 'body=', JSON.stringify(data));
      throw new Error(`Pairing API error: ${errorMsg} (status ${status})`);
    }
    ...
    if (!installationId) {
      ...
      throw new Error('No installation ID returned from Quatt API');
    }

    this._homey.log('Pairing successful, installation ID:', installationId);
